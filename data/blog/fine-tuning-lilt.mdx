...
LiLT is a language-independent Transformer model for document image understanding and information extraction. Put simply, you can use it to build a system that understands receipts and forms. LiLT can be pre-trained on structured documents in one language and then directly fine-tuned on other languages with the corresponding off-the-shelf monolingual/multilingual pre-trained textual models.
...
Having processed our dataset, we can start training our model. We first need to load the [SCUT-DLVCLab/lilt-roberta-en-base](SCUT-DLVCLab/lilt-roberta-en-base) model, which is based on English RoBERTa model with the `LiltForTokenClassification` class with the label mapping of our dataset.
...
Let's save our results and processor to the Hugging Face Hub and create a model card.
...
Additionally, LiLT can easily be adapted to other languages, making it a great model for multilingual document understanding.
