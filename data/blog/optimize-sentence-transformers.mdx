...
1. [Setup Development Environment](#1setup-development-environment)
2. [Convert a Sentence Transformers model to ONNX and create custom Inference Pipeline](#2convert-a-sentence-transformers-model-to-onnx-and-create-custom-inference-pipeline)
3. [Apply graph optimization techniques to the ONNX model](#3apply-graph-optimization-techniques-to-the-onnx-model)
4. [Apply dynamic quantization using `ORTQuantizer` from Optimum](#4apply-dynamic-quantization-using-ortquantizer-from-optimum)
5. [Test inference with the quantized model](#5test-inference-with-the-quantized-model)
6. [Evaluate the performance and speed](#6evaluate-the-performance-and-speed)
